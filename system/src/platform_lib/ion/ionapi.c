...
static __attribute__((unused)) mentry_t *mentry_remove_by_phys(unsigned long phys_addr)
{
    mentry_t *prev = NULL;
    mentry_t *curr = NULL;
...
static void mentry_list(void)
{
    ...
    while(curr != NULL) {
        printf("%s: heap:%s type:%s handle:%08x phys:%08lx request:%x alloc:%x\n", __func__, (curr->heap_type == RTK_SATURN_ION_HEAP_MEDIA_MASK) ? "media" : "contig", (curr->type & 0x1) ? "audio" : "video", (unsigned int)curr->handle, curr->phys_addr, curr->request_size, curr->alloc_size);
        curr = curr->next;
    }
}
              ...
              unsigned int flags, struct ion_handle **handle)
{
    ...
    //printf("%s: len:%u align:%x heap_mask:%x flags:%x\n", __func__, len, align, heap_mask, flags);
...
int ion_phys(int fd, struct ion_handle *handle, unsigned long *addr, unsigned int *size)
{
    int ret;
...
void *ionapi_alloc(unsigned int size, unsigned int align, unsigned int type, void **noncacheable, unsigned long *phys_addr, struct ion_handle **ionhdl)
{
    int ret;
    int mapfd;
...
//    printf("[%s:%d], open fd = %d, pid = %d phys_addr=%lx\n", __FUNCTION__, __LINE__, ion_fd, pid, ent->phys_addr);
    ...
    ionInfo->count++;
    ent->virt_addr = (int)*noncacheable;
...
//void pli_ion_free(void *addr, unsigned int size, struct ion_handle *ionhdl)
void ionapi_free(void *addr, unsigned int size, struct ion_handle *ionhdl)
{
    int ret;
//    long pid = getpid();
...
//    printf("[%s:%d], enter 0x%08x (%d), pid = %d\n", __FUNCTION__, __LINE__, addr, size, pid);
    ...
    mentry_t *ent = mentry_remove_by_virt((int)addr);
    if (ent == NULL)
    {
        printf("[IONDBG] free ent is NULL pid=%ld getpid()=%ld addr:0x%.8x\n", ionInfo->pid, (long)getpid(), (unsigned int)addr);
    }
    else
    {
...
#if 1
        ret = munmap((void*)ent->virt_addr, (size_t)ent->alloc_size);
        if (ret < 0) {
...
{
    mentry_t *prev __attribute__((unused)) = NULL;
    mentry_t *curr __attribute__((unused)) = NULL;
